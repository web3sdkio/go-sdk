...
Once you instantiate the SDK, you can use it to access your web3sdkio contracts. You can use SDK's contract getter functions like `GetNFTCollection`, `GetEdition`, and `GetNFTDrop`, to get the respective SDK contract instances. To use an NFT Collection contract for example, you can do the following.
...
func main() {
	...
	tx, err := nft.Mint(
		&web3sdkio.NFTMetadataInput{
			Name:        "Test NFT",
			Description: "Minted with the web3sdkio Go SDK",
