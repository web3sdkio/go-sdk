...
func TestRejectIncorrectDomain(t *testing.T) {
	...
	sdk.Auth.UpdatePrivateKey(secondaryPrivateKey)
	_, err := sdk.Auth.Verify("test.web3sdk.io", payload, nil)

	assert.Equal(t, err.Error(), "Expected domain 'test.web3sdk.io' does not match domain on payload 'web3sdk.io'")
}
...
func TestRejectTokenIncorrectDoain(t *testing.T) {
	...
	sdk.Auth.UpdatePrivateKey(secondaryPrivateKey)
	token, _ := sdk.Auth.GenerateAuthToken(domain, payload, nil)
	_, err := sdk.Auth.Authenticate("test.web3sdk.io", token)

	assert.Equal(t, err.Error(), "Expected token to be for the domain 'test.web3sdk.io', but found token with domain 'web3sdk.io'")
}
